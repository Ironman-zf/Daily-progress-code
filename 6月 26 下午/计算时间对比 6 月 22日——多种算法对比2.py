...
def thermodynamic_forward_model(n_array, mass_flow, base_des_para):
    ...
    try:
        with warnings.catch_warnings():
            ...
            if not out.get('feasible', False): return np.nan, np.nan, np.nan

            # 🚨 精确对齐用户字典的键值
            power = out.get('Power_in_total_W', np.nan)
            p_comp_array = out.get('P_air_comp', [])
            pressure = p_comp_array[-1] if len(p_comp_array) > 0 else np.nan
            eff = out.get('eta_ex_system', np.nan)
        ...
        try:
            if optimizer_type == 'SLSQP':
                ...
                res = minimize(obj, x0=current_x0, method='trust-constr', bounds=bnds,
                               constraints=(nlc_pow, nlc_pres), options={'xtol': 1e-4, 'eps': 1e-5 ,'maxiter': 666, 'disp': False})
...
def run_benchmark_multi_algo():
    ...
    for current_algo in TARGET_ALGORITHMS:
        ...
        for i in range(start_index, NUM_SAMPLES):
            ...
            # --- 阶段 B：多态统迭代寻优 ---
            # 区分不同算法的重启次数以平衡时间 (DE 耗时极大，内部已有代数控制，n_restarts 无需过大)
            restarts_config = {'SLSQP': 100, 'TRUST': 100, 'DE': 15}
