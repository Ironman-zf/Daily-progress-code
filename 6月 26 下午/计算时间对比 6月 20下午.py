...
def load_and_sample_unseen_data(unseen_file_paths, sample_count=50):
    ...
    for current_file_path in unseen_file_paths:
        ...
        sorted_dataframe['Inlet_Pressure_bar'] = ip_bar
        sorted_dataframe['Flag_S1_Active'] = flag_1
        sorted_dataframe['Flag_S2_Active'] = flag_2
        sorted_dataframe['Flag_S3_Active'] = flag_3
        sorted_dataframe['Flag_S4_Active'] = flag_4

        # 将当前文件清洗后的【全量】有效数据加入全局池，绝不在此处进行局部抽样
        valid_dataframes_list.append(sorted_dataframe)
...
def global_optimization_traditional(target_power, target_pressure, active_flags, base_des_para,
                                    ...
                                    power_tol=0.0001, bp_tol=0.0001): # 💡 建议设为 0.001 (0.1%)
    ...
    for k in range(n_restarts):
        ...
        if k == 0:
            ...
            current_x0 = np.copy(lhs_physical_samples[k - 1])
        try:
            res = minimize(
...
def run_benchmark_only():
    ...
    output_csv_path = BASE_DIR / 'Compressor_Benchmark_FullData8.csv'
    output_fig_path = BASE_DIR / 'Compressor_Benchmark_3Panels8.png'
